function [FigHandle] = plot_2d_prototype_model(X, Y, W, Y_W, Omega, Fig)
...
%   W       ... (optional) A K x 2 matrix of prototypes, where each row is
%               one prototype vector.
%   Y_W     ... (optional) A K x 1 vector of labels, where Y_W(k) is the
%               label for prototype W(k, :)
%   Omega   ... (optional) A 2 x 2 relevance matrix. This matrix can also
...
for l=1:L  % iterate through data classes
    ...
    % plot data in 2D
    plot( X(inClass,1), X(inClass,2), 'LineStyle', 'none', 'Marker', 'o', 'MarkerSize', 5, 'MarkerEdgeColor', squeeze(cm(c,3,:)), 'MarkerFaceColor', squeeze(cm(c,1,:)) );
