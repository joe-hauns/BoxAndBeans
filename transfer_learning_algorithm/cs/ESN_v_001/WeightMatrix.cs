...
namespace EchoState {
	...
	public class WeightMatrix {
		...
		public WeightMatrix(List<Tuple<int, int, double>> entries, int M, int N) {
			this.M = M;
			this.N = N;
			...
			int i = 0;
			int k = 0;
			foreach(Tuple<int, int, double> entry in entries) {
				if(entry.Item1 >= this.M) throw new IndexOutOfRangeException(entry.Item1 + " is out of range of the matrix with " + M + " rows!");
				...
				if(entry.Item2 >= this.N) throw new IndexOutOfRangeException(entry.Item2 + " is out of range of the matrix with " + N + " columns!");
				this.col_idxs[k] = entry.Item2;
				// set the value
				this.values[k]   = entry.Item3;
				k++;
			}
			i++;
			for(;i < M+1; i++) {
		...
		private static int compareTo(Tuple<int, int, double> x, Tuple<int, int, double> y) {
		...
		private static List<Tuple<int, int, double>> fromJSON(JSONArray jEntries) {
			List<Tuple<int, int, double>> entries = new List<Tuple<int, int, double>>();
			for(int k = 0; k < jEntries.Count; k++) {
				entries.Add(new Tuple<int, int, double>(jEntries[k][0].AsInt - 1, jEntries[k][1].AsInt - 1, jEntries[k][2].AsDouble));
			}
			return entries;
		}
