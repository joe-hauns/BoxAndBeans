...
/*
		...
		public static void Main() {
			...
			// use a Buffer to cut it into pieces and run the network on-line
			int t = 0;
			Buffer buf = new Buffer();
			// Also use a CombFilter for powerline interference cleanup
			CombFilter filter = new CombFilter(200, 50);
			for(int i = 0; i < raw_data.GetLength(0); i++) {
				if(buf.store(DenseMatrix.Row(raw_data, i))) {
